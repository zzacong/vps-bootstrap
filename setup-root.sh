...
if [[ "$NEW_USER" == "root" ]] || [[ "$NEW_USER" =~ ^(daemon|bin|sys|sync|games|man|lp|mail|news|uucp|proxy|www-data|backup|list|irc|gnats|nobody|nobody4|halt|shutdown|operator|systemd-.*|_.*)$ ]]; then
