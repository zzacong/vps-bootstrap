...
2. **Shell env** — oh-my-zsh (unattended), autosuggestions + syntax-highlighting plugins, starship prompt (installed via its official script).
...
3. **Shell env** — oh-my-zsh (unattended), autosuggestions + syntax-highlighting plugins, starship prompt. zsh is already the default on macOS, so no `chsh` needed.
